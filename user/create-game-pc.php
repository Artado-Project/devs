...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category = 'pc_oyun'; // Kategori sabit olarak eklenti
    ...
    if (isset($_FILES['file'])) {
        ...
        if ($fileError === UPLOAD_ERR_OK) {
            ...
            // Dizin yoksa oluştur ve izinleri ayarla
            if (!file_exists($target_dir)) {
                mkdir($target_dir, 0777, true);
                chmod($target_dir, 0777);
            } else {
                // Mevcut dizinin izinlerini kontrol et ve ayarla
                chmod($target_dir, 0777);
            }
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
                ...
                try {
                    ...
                    if (is_string($image_upload_result) && strpos($image_upload_result, 'public/uploads/img/') === 0) {
                        // Resim başarıyla yüklendi, project_images tablosuna ekle
                        ...
                        $stmt_image->bindParam(':image_path', $image_db_path);
                        $stmt_image->bindParam(':project_id', $project_id);
                        if (!$stmt_image->execute()) {
                             error_log("Proje resmi DB INSERT hatası (Game/Uyg): Project ID " . $project_id . " - Resim Yolu: " . $image_db_path); 
                        } 
                    }
                    ...
                    if (!isset($_SESSION['warning_message']) && !isset($_SESSION['error_message'])) { // Hata veya uyarı yoksa başarı mesajı
                        $_SESSION['success_message'] = "PC Oyunu başarıyla oluşturuldu.";
                    }
                     // Yönlendirme için doğru kategoriyi kullan
                    header("Location: projects.php?category=pc_oyun");
                    exit();
