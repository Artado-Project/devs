...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category = 'mobil_oyun'; // Kategori sabit olarak eklenti
                ...
                try {
                    ...
                    if (is_string($image_upload_result) && strpos($image_upload_result, 'public/uploads/img/') === 0) {
                        ...
                        if (!$stmt_update->execute()) {
                             error_log("Proje resmi DB UPDATE hatası (Mobil Oyun): Project ID " . $project_id . " - Resim Yolu: " . $image_db_path); 
                             $_SESSION['error_message'] = "Mobil Oyun oluşturuldu ancak resim veritabanına kaydedilirken bir sorun oluştu."; 
                        } 
                    } elseif ($image_upload_result !== null) {
                         error_log("Proje resmi yükleme hatası (Mobil Oyun - Project ID: $project_id): " . $image_upload_result);
                         $_SESSION['warning_message'] = "Mobil Oyun oluşturuldu ancak resim yüklenirken bir sorun oluştu: " . $image_upload_result;
                    }
                    ...
                    if (!isset($_SESSION['warning_message']) && !isset($_SESSION['error_message'])) { // Hata veya uyarı yoksa başarı mesajı
                        $_SESSION['success_message'] = "Mobil Oyun başarıyla oluşturuldu.";
                    }
                    // Yönlendirme için doğru kategoriyi kullan
                    header("Location: projects.php?category=mobil_oyun"); 
                    exit();
