...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category = 'mobil_uygulama'; // Kategori sabit olarak eklenti

    // Dosya yükleme işlemleri
    if (isset($_FILES['file'])) {
        $fileError = $_FILES['file']['error'];
        if ($fileError === UPLOAD_ERR_OK) {
            $target_dir = "../public/uploads/files/"; // Yükleme dizini public altına taşındı
            
            // Hedef dizin yoksa oluştur (İzin 0755)
            if (!file_exists($target_dir)) {
                if (!mkdir($target_dir, 0755, true)) {
                    $error = "Proje dosyası yükleme dizini oluşturulamadı.";
                    goto upload_error; 
                }
            }
            
            $file_name = basename($_FILES["file"]["name"]); // Orijinal adı koruyoruz
            $target_file = $target_dir . $file_name;

            if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
                // Dosya İzinlerini Ayarla (0644)
                if (!chmod($target_file, 0644)) {
                    error_log("Proje dosyası chmod başarısız: " . $target_file); // Hata loglama eklendi
                }
                
                // Veritabanına yolu kaydet (web kök dizinine göre public/ ile başlayan yol)
                $db_file_path = 'public/uploads/files/' . $file_name;
                $image_db_path = null; // Resim yolu için başlangıç değeri
                
                try {
                    // 1. Adım: Proje bilgilerini (resim hariç) veritabanına kaydet
                    ...
                    $project_id = $db->lastInsertId(); // Yeni proje ID'sini al
                    ...
                    // 2. Adım: Proje resmini yüklemeyi dene
                    $image_upload_result = uploadProjectImage(); // Argümansız çağır

                    if (is_string($image_upload_result) && strpos($image_upload_result, 'public/uploads/img/') === 0) {
                        // Resim başarıyla yüklendi, veritabanını güncelle
                        $image_db_path = $image_upload_result;
                        $stmt_update = $db->prepare("UPDATE projects SET image_path = :image_path WHERE id = :project_id");
                        $stmt_update->bindParam(':image_path', $image_db_path);
                        $stmt_update->bindParam(':project_id', $project_id);
                        if (!$stmt_update->execute()) {
                             error_log("Proje resmi DB UPDATE hatası (Mobil Uyg): Project ID " . $project_id . " - Resim Yolu: " . $image_db_path); 
                             $_SESSION['error_message'] = "Mobil Uygulama oluşturuldu ancak resim veritabanına kaydedilirken bir sorun oluştu."; 
                        } 
                    } elseif ($image_upload_result !== null) {
                         error_log("Proje resmi yükleme hatası (Mobil Uyg - Project ID: $project_id): " . $image_upload_result);
                         $_SESSION['warning_message'] = "Mobil Uygulama oluşturuldu ancak resim yüklenirken bir sorun oluştu: " . $image_upload_result;
                    }

                    // 3. Adım: Başarı mesajı ve yönlendirme
                    if (!isset($_SESSION['warning_message']) && !isset($_SESSION['error_message'])) { // Hata veya uyarı yoksa başarı mesajı
                        $_SESSION['success_message'] = "Mobil Uygulama başarıyla oluşturuldu.";
                    }
                     // Yönlendirme için doğru kategoriyi kullan
                    header("Location: projects.php?category=mobil_uygulama");
                    exit();
                ...
                $error = "Dosya taşınırken bir hata oluştu (muhtemelen yazma izinleri).";
            }
        } else {
            switch ($fileError) {
                            ...
                            <form method="post" enctype="multipart/form-data">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="form-group">
                                            <label for="file">Eklenti Dosyası (.zip)</label>
                                        ...
                                        <div class="form-group">
                                            <label for="image">Eklenti Resmi</label>
                                            <input type="file" class="form-control" id="image" name="image" accept="image/*">
                                            <small class="text-muted">Önerilen boyut: 800x400 piksel</small>
                                        </div>
                                    </div>
                                </div>

                                <div class="col-12 d-flex justify-content-end">
                                    <button type="submit" class="btn btn-primary me-1 mb-1">Eklentiyi Oluştur</button>
