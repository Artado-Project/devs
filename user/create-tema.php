...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category = $_POST['category'] ?? 'artado_tema'; // Yeni eklendi - kategori seçimi
    ...
    try {
        ...
        $stmt = $db->prepare("INSERT INTO projects (user_id, title, description, version, features, category, approval_status, is_private) 
                            VALUES (:user_id, :title, :description, :version, :features, :category, 'pending', 0)");
        $stmt->execute([
            ':user_id' => $user_id,
            ':title' => $title,
        ...
        if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($file_extension, $allowed_extensions)) {
                throw new Exception("Hata: Temalar için sadece .css dosyaları yüklenebilir.");
            }
                    ...
                    <ul class="menu">
                        ...
                        <li class="sidebar-item has-sub active">
                            ...
                            <ul class="submenu">
                                ...
                                <li class="submenu-item">
                                    <a href="create-game-mobil.php" class="submenu-link">Mobil Oyunu</a>
                                </li>
                            </ul>
                        </li>
                ...
                <div class="page-title">
                    <div class="row">
                        <div class="col-12 col-md-6 order-md-1 order-last">
                            <h3>Tema Oluştur</h3>
                            <p class="text-subtitle text-muted">Artstelve için yeni bir tema oluşturun.</p>
                        </div>
                        <div class="col-12 col-md-6 order-md-2 order-first">
                            <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="index.php">Ana Sayfa</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">Tema Oluştur</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                ...
                <section class="section">
                    <div class="row">
                        <div class="col-md-8">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">Yeni Tema Bilgileri</h4>
                                </div>
                                <div class="card-body">
                                    <?php if (isset($error)): ?>
                                    ...
                                    <form method="post" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="title">Tema Adı</label>
                                            <input type="text" class="form-control" id="title" name="title" required placeholder="Midnight Purple">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="category" name="category" required>
                                                ...
                                                <option value="artado_tema">Site Teması</option>
                                                <option value="ana_sayfa">Ana Sayfa Teması</option>
                                            </select>
                                            <small class="text-muted">Temanın türünü seçin. Site temaları tüm siteyi etkiler, ana sayfa temaları sadece ana sayfayı etkiler.</small>
                                        </div>

                                        <div class="form-group">
                                            <label for="description">Açıklama</label>
                                            <textarea class="form-control" id="description" name="description" rows="4" required placeholder="Artstelve için koyu mor cam efektli tema."></textarea>
                                        </div>

                                        <div class="form-group">
                                            <label for="file">Tema Dosyası (.css)</label>
                                            <input type="file" class="form-control" id="file" name="file" accept=".css" required>
                                            <small class="text-muted">Sadece CSS dosyaları kabul edilir.</small>
                                        </div>

                                        <div class="form-group">
                                            <label for="image">Önizleme Resmi</label>
                                            <input type="file" class="form-control" id="image" name="image" accept="image/*">
                                        </div>

                                        <div class="mt-4 d-flex justify-content-end">
                                            <button type="submit" class="btn btn-primary me-1 mb-1">Temayı Yayınla</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title"><i class="bi bi-info-circle"></i> Artstelve Tema Rehberi</h4>
                                </div>
                                <div class="card-body">
                                    <h6>1. Kategori Seçimi</h6>
                                    <p class="text-sm">Temanızın türünü seçin. Site temaları tüm siteyi etkilerken, ana sayfa temaları sadece arama ekranını etkiler.</p>
                                    
                                    <h6>2. Tek Dosyalı Temalar (.css)</h6>
                                    <p class="text-sm">Sadece bir CSS dosyası yükleyerek hızlıca tema oluşturabilirsiniz. Artstelve bu dosyayı otomatik olarak tanıyacaktır.</p>
                                    
                                    <h6>3. Gelişmiş Temalar (.zip)</h6>
                                    <p class="text-sm">Eğer temanız birden fazla dosya veya resim içeriyorsa, hepsini bir ZIP dosyasına koyup yükleyin.</p>
                                    
                                    <h6>4. Önemli İpucu</h6>
                                    <p class="text-sm">Temanızın adı ile klasör adının aynı olması önerilir. ZIP içeriğinde en az bir adet CSS dosyası bulunmalıdır.</p>
                                    
                                    <div class="alert alert-light-info">
                                        <p class="mb-0"><strong>Not:</strong> Temanız yayınlandıktan sonra Artstelve Atölye sekmesinde anında görünecektir.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
