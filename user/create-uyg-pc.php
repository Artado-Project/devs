...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $category = 'pc_uygulama'; // Kategori sabit olarak eklenti
                ...
                try {
                    ...
                    if (is_string($image_upload_result) && strpos($image_upload_result, 'public/uploads/img/') === 0) {
                        ...
                        if (!$stmt_update->execute()) {
                             error_log("Proje resmi DB UPDATE hatası (PC Uyg): Project ID " . $project_id . " - Resim Yolu: " . $image_db_path); 
                             $_SESSION['error_message'] = "PC Uygulaması oluşturuldu ancak resim veritabanına kaydedilirken bir sorun oluştu."; 
                        } 
                    } elseif ($image_upload_result !== null) {
                         error_log("Proje resmi yükleme hatası (PC Uyg - Project ID: $project_id): " . $image_upload_result);
                         $_SESSION['warning_message'] = "PC Uygulaması oluşturuldu ancak resim yüklenirken bir sorun oluştu: " . $image_upload_result;
                    }
                    ...
                    if (!isset($_SESSION['warning_message']) && !isset($_SESSION['error_message'])) { // Hata veya uyarı yoksa başarı mesajı
                        $_SESSION['success_message'] = "PC Uygulaması başarıyla oluşturuldu.";
                    }
                     // Yönlendirme için doğru kategoriyi kullan
                    header("Location: projects.php?category=pc_uygulama");
                    exit();
